...
## 📌 개발 배경
스마트팜 환경 데이터를 수집·관리하는 플랫폼을 가정해, 단순 모니터링을 넘어 농민이 자연어로 질문하면 데이터 기반으로 답하고 이상값을 사전에 감지할 수 있는 구조를 직접 검증해보고자 만들었습니다. 
실제 농장 데이터 없이 구조 검증을 목적으로 설계했으며, 실제 센서 데이터가 있다면 동일한 구조로 연결할 수 있을 것 같습니다.
...
정상 600건 / 이상 400건으로 구성했습니다. 이상 패턴을 충분히 학습시키기 위해 이상 비율을 의도적으로 높였습니다. 
다양한 케이스를 커버하기 위해 정상과 이상의 경계값을 의도적으로 겹치게 설정했습니다. 
초기 RAG 구현에서 한국어 응답이 불안정해 프롬프트에 언어와 문서 범위를 명시하는 방식으로 개선했습니다.
